...
def eye_aspect_ratio(eye: np.ndarray) -> float:
    """Compute the eye aspect ratio (EAR) for a given eye landmarks array."""
...
def get_gaze_ratio(eye_frame: np.ndarray, thresh_val: int = 70) -> float:
    """
    Threshold the eye crop, apply morphological cleanup,
    and compute the ratio of white pixels in left vs. right halves.
    """
    gray = cv2.cvtColor(eye_frame, cv2.COLOR_BGR2GRAY)
    _, thresh = cv2.threshold(gray, thresh_val, 255, cv2.THRESH_BINARY_INV)

    # morphological open with a 3×3 elliptical kernel
